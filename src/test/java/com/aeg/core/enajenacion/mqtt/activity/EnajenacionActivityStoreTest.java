...
class EnajenacionActivityStoreTest {
	...
	@Test
	void keepsRecentEntriesInNewestFirstOrder() {
		for (int i = 0; i < 5; i++) {
	...
	@Test
	void dropsOldestEntriesWhenCapacityExceeded() {
		InMemoryEnajenacionActivityPersistence persistence = new InMemoryEnajenacionActivityPersistence();
		EnajenacionActivityStore capped = new EnajenacionActivityStore(persistence);

		int overflow = 50;
		for (int i = 0; i < InMemoryEnajenacionActivityPersistence.MAX_ENTRIES + overflow; i++) {
			capped.record(EnajenacionActivityEntry.create(
					"MAC" + i,
					null,
					null,
					EnajenacionActivityDirection.INBOUND,
		...
		assertThat(capped.count(EnajenacionActivityQuery.unrestricted()))
				.isEqualTo(InMemoryEnajenacionActivityPersistence.MAX_ENTRIES);
		assertThat(capped.find(EnajenacionActivityQuery.unrestricted(), 1, 0).get(0).mac())
				.isEqualTo("MAC" + (InMemoryEnajenacionActivityPersistence.MAX_ENTRIES + overflow - 1));
	}
